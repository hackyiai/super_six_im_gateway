...
public  class JedisTemplate implements  Serializable{  
	...
	private static Jackson2JsonRedisSerializer<Object> jackson2JsonRedisSerializer = null;
	...
	private static final void init() throws Exception {
		...
		try {
		    if (StringUtils.isEmpty(redisConfig.getAuth())) {
                jedisPool = new JedisPool(poolConfig, redisConfig.getHost(),redisConfig.getPort(), redisConfig.getTimeout(),null,redisConfig.getDataBase());
            } else {
                jedisPool = new JedisPool(poolConfig, redisConfig.getHost(), redisConfig.getPort(),  redisConfig.getTimeout(),  redisConfig.getAuth(),redisConfig.getDataBase());
            }
		} catch (Exception e) {
    ...
    private static final void initSerializer(){
   ...
   public List<String> listRange(final String key, final long beginIndex, final long endIndex) {  
     ...
     * @return
     */
    public Set<String> setGetAll(final String key) {
        return new Executor<Set<String>>(jedisPool) {
            @Override
            Set<String> execute() {
   ...
   public class PairEx<K,V,E> extends Pair<K, V>{
    ...
    public Object serializerString(String str){
        Object result = null;
        try {
            result = jackson2JsonRedisSerializer.deserialize(str.getBytes());
        } catch (Exception e) {
            logger.error("string 序列化异常e:{}",e);
        }
        return result;
    }
}
