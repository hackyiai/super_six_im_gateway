...
public class ChatReqHandler extends AbCmdHandler {
	...
	public ImPacket handler(ImPacket packet, ChannelContext channelContext) throws Exception {
		...
		//新增拦截
		ChatProcessorIntf chatProcessorIntf = (ChatProcessorIntf)this.getProcessor(channelContext);
		if(null != chatProcessorIntf){
			RespBody respBody = chatProcessorIntf.beforeHandler(chatBody, channelContext);
			if(null == respBody){
				ImPacket respChatPacket = ChatKit.dataInCorrectRespPacket(channelContext);
				return respChatPacket;
			}
			...
			if(null == respBody.getData()){
				ImAio.send(channelContext, new ImPacket(Command.COMMAND_CHAT_REQ,respBody.toByte()));
				return null;
			}
		}
